...
router.post('/googleapi', async (req, res) => {
  try {
    ...
    if (!checkEmail(req.body.client_email)) {
      return res.status(400).send({
        message: 'Client email must be a valid email address for the service account.'
      });
    }

    if (!checkPrivateKey(req.body.private_key)) {
      return res.status(400).send({
        message: 'Private key must be a valid private key for the service account.'
      });
    }

    const comparePassword = await bcrypt.compareSync(req.body.password, req.user.password);
    if (!comparePassword) {
