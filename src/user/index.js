  ...
  try {
    if (req.user.rank < 3) {
      return res.status(403).send({
        message: 'Only admin users and teacher users can set ' + 'user ranks.'
      });
    }
    ...
    if (req.user.rank < 4 && parseInt(user.rank) > 2) {
      return res.status(403).send({
        message: 'Only admin users can change the rank of an ' + 'admin or a teacher.'
      });
    }

    if (req.user.rank < 4 && parseInt(req.body.rank) > 2) {
      return res.status(403).send({
        message: "Only admin users can set a user's rank to " + 'admin or teacher.'
      });
    }
...
router.post('/assistant-request', getEmailCredentials, async (req, res) => {
  ...
  try {
    await beAssistantRequest(req.transport, req.body.teacherEmail, req.body.event, user);
    return res.send({
      message: 'Request email sent. Now wait for a teacher to accept your request.'
    });
  } catch (error) {
    return res.status(500).send({
      message: 'Internal server error.'
